...
export default function PDFExport({
  ...
  const generatePDF = () => {
    ...
    const addText = (text: string, x: number, y: number, options: any = {}) => {
      ...
      doc.text(lines, x, y, { letterSpacing: 0.1 });
    ...
    const addSectionHeader = (title: string, y: number) => {
      ...
      doc.text(title, 20, y, { letterSpacing: 0.1 });
    ...
    doc.setFont("arial", "bold");
    doc.setTextColor(0, 0, 0);
    doc.text("Slope Drainage Channel Design Report", pageWidth / 2, yPosition, { align: "center", letterSpacing: 0.1 });
    yPosition += 15;

    doc.setFontSize(12);
    doc.setFont("arial", "normal");
    doc.text(`Channel Number: ${channelNo || "Not specified"}`, 20, yPosition, { letterSpacing: 0.1 });
    yPosition += 8;
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, yPosition, { letterSpacing: 0.1 });
    yPosition += 8;
    doc.text(`Time: ${new Date().toLocaleTimeString()}`, 20, yPosition, { letterSpacing: 0.1 });
    yPosition += 15;
    ...
    autoTable(doc, {
      ...
      startY: yPosition,
      styles: { fontSize: 9, letterSpacing: 0.1 },
      headStyles: { fillColor: [66, 139, 202], letterSpacing: 0.1 },
    });
    ...
    doc.text("Generated by Slope Drainage Design Tool", pageWidth / 2, finalY, { align: "center", letterSpacing: 0.1 });
    doc.text("Based on Rational Method and Manning's Equation", pageWidth / 2, finalY + 5, { align: "center", letterSpacing: 0.1 });
